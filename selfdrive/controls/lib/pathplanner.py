...
LOG_MPC = os.environ.get('LOG_MPC', False) #Ture
...
class PathPlanner():
  ...
  def update(self, sm, pm, CP, VM):
    ...
    VM.update_params(sm['liveParameters'].stiffnessFactor, CP.steerRatio)  #sR 고정
    ...
    if (not active) or (self.lane_change_timer > LANE_CHANGE_TIME_MAX) or (not one_blinker) or (not self.lane_change_enabled):
      ...
      if self.lane_change_direction == LaneChangeDirection.left:
        ...
        if CP.autoLcaEnabled and 0.5 > self.pre_auto_LCA_timer > 0.2 and not lca_left:
          torque_applied = True # Enable auto LCA only once after 2 sec 
      else:
        torque_applied = sm['carState'].steeringTorque < 0 and sm['carState'].steeringPressed
        if CP.autoLcaEnabled and 0.5 > self.pre_auto_LCA_timer > 0.2 and not lca_right:
          torque_applied = True # Enable auto LCA only once after 2 sec 
